...
public class PGNDisplay extends JPanel implements Serializable
{
	...
	//Rook r = new Rook(autoscrolls, null);
	...
	public void updatePGN(Location start, Location end, boolean turn)
	{
		...
		if(turn) //white piece
		{
			...
			if(pgn.size()>99) 
			{
				...
				System.out.println("Piece: " + colorOfPiece  + "Initial Position: " + start + " End Position: " + end);
			}
			else if(pgn.size()>9)
			{
				pgnLabel.setText(pgn.size()+". "+start+" "+end+"  :         ");
		...
		else //black piece goes
		{
			if(pgn.size()>99)
			{
				...
				System.out.println("Piece: " + colorOfPiece  +"INITAL Position: " + start + " End Position: " + end);
			}
			else if(pgn.size()>9)
			{
				pgnLabel.setText(temp.trim()+"  "+start+" "+end+"  ");
