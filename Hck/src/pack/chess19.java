...
public class chess19 extends java.applet.Applet implements MouseMotionListener
{
	...
	public void init()
	{
		...
		for (int k = 0; k < 4; k++)
		{
			for (int l = 0; l < 8; l++)
			{
				...
				if (pieces[counter].name.equals("King"))
	...
	class MousePressListener extends MouseAdapter
	{
		...
		public boolean KingRules(int y, int x, boolean piecefound, int pieceNum)
		{
			...
			if (pieces[pieceChosen].name.equals("King"))
			{
				y = y / 50 + 1;
				x = x / 50 + 1;
				if (pieces[pieceChosen].column == y || pieces[pieceChosen].column == y + 1 || pieces[pieceChosen].column == y - 1)
				{
					if (pieces[pieceChosen].column == x || pieces[pieceChosen].column == x + 1 || pieces[pieceChosen].column == x - 1)
					{
						if (!piecefound)
						{
							canmove = true;
		...
		public void MovePiece(int x, int y)
		{
			...
			if (PawnRules(x,y,foundpiece,pieceNum) ||  KingRules(x,y,foundpiece,pieceNum))
			{
