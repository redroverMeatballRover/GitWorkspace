...
//CHess 22
//pawn rules for movement, check, sqaures attacked
//knight rules for movement, check
...
public class chess22 extends java.applet.Applet implements MouseMotionListener
{
	...
	/*class attacked
	{
		int wsquares[][] = new int [8][8];

		public int cyclethroughpieces()
		{
			for (int i = 0; i < 32; i++)
			{
				pieces[i].row;
	...
	class MousePressListener extends MouseAdapter
	{
		...
		public boolean WhiteKingChecked()
		{
			for (int i = 8; i < 16; i++)
			{
				if (pieces[i].column == 1)
					wsquares[pieces[i].row][pieces[i].column] = 2;
				else if (pieces[i].column == 8)
					wsquares[pieces[i].row][pieces[i].column-2] = 2;
				else
				{
					wsquares[pieces[i].row][pieces[i].column] = 2;
