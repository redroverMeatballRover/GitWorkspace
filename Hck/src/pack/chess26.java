...
public class chess26 extends java.applet.Applet implements MouseMotionListener
{
	...
	class MousePressListener extends MouseAdapter
	{
		...
		public boolean UpdateCoordinates(int coor[], int movecase)
		{
			...
			/*if (num != 40)
			//{
				//if (pieces[num].color.equals("white"))
				//{
		...
		public boolean SquaresAttacked()
		{
			...
			for (int i = 24; i < 32; i++)
			{
				...
				oldy = tempcoor[1];
				num = i;
				if (pieces[i].name.equals("Bishop"))
				{
					for (int k = 0; k < 3; k++)
					{
						//here:
						...
						for (int l = 0; l < 7; l++)
						{
							if (UpdateCoordinates(tempcoor, k))
							{
								x = tempcoor[0];
								y = tempcoor[1];
								if (piecethere)
									break;
								for (int m = 0; m < 32; m++)
									if (pieces[m].row == y && pieces[m].column == x)
										piecethere = true;
								if (wsquares[x][y] == 2)
									wsquares[x][y] += 1;
								else
